#!/bin/sh
# Baut den Signal-Dienst fuer das N9/N950.
#
#   tools/build.sh          # -> build/signal-backend
#
...
mkdir -p build
scp -q "$HOST:$FERN/backend/target/armv7-unknown-linux-musleabi/release/signal-backend" \
    build/
echo "== signal-backend fertig ($(stat -c %s build/signal-backend) B)"
