...
# -*- coding: utf-8 -*-
"""Adapter zwischen pybridge und dem Signal-Dienst.
...
Dieser Daemon uebersetzt das auf die HTTP-Schnittstelle des Rust-Dienstes.
Er spricht bewusst die Datenform des Telegram-Daemons, denn der Manager
verzweigt nach Protokoll -- und was er fuer Telegram tut, passt auch hier:
Chat-Kennung, Absender, Text, Zeitstempel. So bleibt der Eingriff in
fremden Code auf ein paar Zeilen beschraenkt.
...
# Die Ports, die sich der Dienst nimmt (siehe backend/src/main.rs).
PORTS = (8095, 8096, 8097, 8098, 8099)
DBUS_NAME = 'org.smatkovi.Fluesterwind'
...
class Backend(object):
    ...
    def wecke(self):
        """Bittet den Sitzungs-D-Bus, den Dienst zu starten.

        Nicht selbst starten -- die Aktivierung ueber den Bus ist der
        einzige Weg, der garantiert genau eine Instanz ergibt.

        Und nicht ueber Peer.Ping: der Dienst beansprucht keinen
        D-Bus-Namen (er braucht dafuer keine D-Bus-Bibliothek in Rust),
        also antwortet auf ein Ping niemand, und der Aufruf laeuft in
        seine Zeitgrenze. StartServiceByName dagegen startet den Dienst
        und kehrt zurueck, sobald das geschehen ist.
        """
        try:
            subprocess.call(
...
class Daemon(object):
    ...
    def _namen_auffrischen(self):
        # Anders als beim WhatsApp-Dienst gibt es hier keinen eigenen
        # /contacts-Pfad: der Signal-Dienst loest Absendernamen schon auf,
        # und die Chatliste traegt die Titel. Bleibt nichts nachzuschlagen.
        pass
